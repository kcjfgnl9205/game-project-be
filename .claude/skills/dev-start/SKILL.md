...
0. 위 "일지 파일 목록"에서 오늘 날짜가 아닌 가장 최근 `.md` 파일을 골라 Read 도구로 읽는다. 파일이 없으면 "이전 일지 없음 — 첫 작업일"로 간주하고 다음 단계로 넘어간다.
1. 그 가장 최근 일지에서 **"➡️ 내일 할 일"** 섹션과 미완료(체크 안 된) 항목을 찾아 요약한다.
